...
const ITCC47LinearADTActivities = (() => {
    ...
    const events = spec.steps.map((step, index) => {
      ...
      return ITCC47Playback.timelineEvent({
        ...
        frame: {
          ...
          ...(step.presentation ? { presentation: Object.freeze({
            ...step.presentation,
            phases: Object.freeze([...(step.presentation.phases || [])]),
            discarded: step.presentation.discarded ? Object.freeze({ ...step.presentation.discarded }) : null,
          }) } : {}),
  ...
  const slidingWindow = buildActivity({
    ...
    input:{label:'Input array + sliding window',tokens:['4','2','12','3'],active:0},
    source:[
      'deque ← empty',
      'FOR i ← 0 TO 3 DO',
      '  WHILE deque not empty AND',
      '        values[back] ≤ values[i] DO',
      '    REMOVE_BACK deque',
      '  ENDWHILE',
      '  ADD_BACK deque, i',
      '  IF front is outside window THEN',
      '    REMOVE_FRONT deque',
      '  ENDIF',
      '  IF i ≥ 2 THEN WRITE values[front]',
      'ENDFOR',
    ],
    complexity:{best:'O(n)',avg:'O(n)',worst:'O(n)',space:'O(k)'},
    steps:[
      {
        line:1,title:'Initialize candidate deque',message:'Start with an empty deque. Store candidate indices, not every value.',
        lanes:[{id:'main',label:'Maximum candidates',kind:'deque',order:[]}],input:{label:'Input array + sliding window',tokens:['4','2','12','3'],active:0},
        operation:{label:'Initialize'},status:[{label:'invariant',value:'front is largest',tone:'secondary'}],variables:{i:0},operations:0,
        presentation:{index:0,operationLabel:'Initialize',activePhase:0,},
      },
      {
        line:7,title:'Add index 0 to BACK',message:'Index 0 enters the candidate deque; value 4 is the first possible maximum.',
        lanes:[{id:'main',label:'Maximum candidates',kind:'deque',order:['v4']}],input:{label:'Input array + sliding window',tokens:['4','2','12','3'],active:0},
        focus:[{id:'v4',label:'front = maximum',tone:'minimum'}],operation:{label:'ADD_BACK i0',end:'back'},variables:{i:0,front:4},operations:1,
        presentation:{index:0,operationLabel:'ADD_BACK i0',activePhase:1,},
      },
      {
        line:3,title:'Compare BACK with incoming 2',message:'Check whether the current BACK is dominated by the newer value.',type:'comparison',
        lanes:[{id:'main',label:'Maximum candidates',kind:'deque',order:['v4']}],input:{label:'Input array + sliding window',tokens:['4','2','12','3'],active:1},
        focus:[{id:'v4',label:'back = 4'}],comparison:{text:'4 ≤ 2',outcome:false},operation:{label:'Compare BACK'},variables:{i:1,back:4},operations:1,comparisons:1,boundary:true,
        presentation:{index:1,operationLabel:'Compare BACK',activePhase:3,},
      },
      {
        line:7,title:'Add index 1 to BACK',message:'The smaller value remains useful behind 4, preserving a decreasing candidate deque.',
        lanes:[{id:'main',label:'Maximum candidates',kind:'deque',order:['v4','v2']}],input:{label:'Input array + sliding window',tokens:['4','2','12','3'],active:1},
        focus:[{id:'v4',label:'maximum',tone:'minimum'},{id:'v2',label:'new back'}],operation:{label:'ADD_BACK i1',end:'back'},variables:{i:1,front:4,back:2},operations:2,comparisons:1,
        presentation:{index:1,operationLabel:'ADD_BACK i1',activePhase:2,},
      },
      {
        line:3,title:'Compare BACK 2 with incoming 12',message:'The newer 12 is larger, so the BACK candidate 2 is dominated.',type:'comparison',
        lanes:[{id:'main',label:'Maximum candidates',kind:'deque',order:['v4','v2']}],input:{label:'Input array + sliding window',tokens:['4','2','12','3'],active:2},
        focus:[{id:'v2',label:'dominated back',tone:'danger'}],comparison:{text:'2 ≤ 12',outcome:true},operation:{label:'Dominate test',end:'back'},variables:{i:2,back:2},operations:2,comparisons:2,boundary:true,
        presentation:{index:2,operationLabel:'Dominate test',activePhase:3,removeId:'v2',},
      },
      {
        line:5,title:'Remove dominated BACK candidate',message:'Remove index 1, then repeat the WHILE comparison against the new BACK.',
        lanes:[{id:'main',label:'Maximum candidates',kind:'deque',order:['v4']}],input:{label:'Input array + sliding window',tokens:['4','2','12','3'],active:2},
        held:[{id:'discarded-v2',label:'dominated by 12',value:'2 · i1',tone:'danger'}],focus:[{id:'v4',label:'compare next'}],operation:{label:'REMOVE_BACK i1',end:'back'},variables:{i:2,back:4},operations:3,comparisons:2,
        presentation:{index:2,operationLabel:'REMOVE_BACK i1',activePhase:2,discarded:{value:'2 · i1',reason:'dominated by 12'},},
      },
      {
        line:3,title:'Repeat comparison: 4 vs 12',message:'After removing 2, compare the new BACK again. The older 4 is also dominated.',type:'comparison',
        lanes:[{id:'main',label:'Maximum candidates',kind:'deque',order:['v4']}],input:{label:'Input array + sliding window',tokens:['4','2','12','3'],active:2},
        focus:[{id:'v4',label:'dominated back',tone:'danger'}],comparison:{text:'4 ≤ 12',outcome:true},operation:{label:'Repeat WHILE',end:'back'},variables:{i:2,back:4},operations:3,comparisons:3,boundary:true,
        presentation:{index:2,operationLabel:'Repeat WHILE',activePhase:3,removeId:'v4',},
      },
      {
        line:7,title:'Add 12, then emit the first maximum',message:'With smaller BACK candidates gone, index 2 becomes the sole candidate and FRONT maximum.',
        lanes:[{id:'main',label:'Maximum candidates',kind:'deque',order:['v12']}],input:{label:'Input array + sliding window',tokens:['4','2','12','3'],active:2},
        focus:[{id:'v12',label:'window maximum',tone:'minimum'}],operation:{label:'ADD + OUTPUT',end:'back'},output:['12'],variables:{i:2,front:12},operations:6,comparisons:3,
        presentation:{index:2,operationLabel:'ADD + OUTPUT',activePhase:3,},
      },
      {
        line:3,title:'Compare BACK 12 with incoming 3',message:'The window slides right. Since 12 is larger, it remains the best candidate.',type:'comparison',
        lanes:[{id:'main',label:'Maximum candidates',kind:'deque',order:['v12']}],input:{label:'Input array + sliding window',tokens:['4','2','12','3'],active:3},
        focus:[{id:'v12',label:'back = 12'}],comparison:{text:'12 ≤ 3',outcome:false},operation:{label:'Compare BACK'},output:['12'],variables:{i:3,back:12},operations:6,comparisons:4,boundary:true,
        presentation:{index:3,operationLabel:'Compare BACK',activePhase:3,},
      },
      {
        line:7,title:'Add 3 and emit the second maximum',message:'Keep 3 behind 12. It may become useful later after 12 eventually expires.',
        lanes:[{id:'main',label:'Maximum candidates',kind:'deque',order:['v12','v3']}],input:{label:'Input array + sliding window',tokens:['4','2','12','3'],active:3},
        focus:[{id:'v12',label:'maximum',tone:'minimum'},{id:'v3',label:'new back'}],operation:{label:'ADD + OUTPUT',end:'back'},output:['12','12'],variables:{i:3,front:12,back:3},operations:7,comparisons:4,
        presentation:{index:3,operationLabel:'ADD + OUTPUT',activePhase:3,},
      },
      {
        line:11,title:'Final maxima complete',message:'Both windows are resolved. The deque remains monotonic and FRONT produced each maximum.',type:'return',
        lanes:[{id:'main',label:'Maximum candidates',kind:'deque',order:['v12','v3']}],input:{label:'Input array + sliding window',tokens:['4','2','12','3'],active:3},
        focus:[{id:'v12',label:'front / maximum',tone:'minimum'}],comparison:{text:'deque values decrease front → back',outcome:true},operation:{label:'Final output'},output:['12','12'],status:[{label:'result',value:'[12, 12]',tone:'success'}],variables:{i:3,windows:2},operations:7,comparisons:5,
        presentation:{index:3,operationLabel:'Final output',activePhase:3,},
      },
    ],result:{maxima:[12,12]},
  });
