...
const roundRobinActivity = Activities.get('queue-round-robin');
const roundRobinEvents = roundRobinActivity.run().events;
const rrPhases = (line, turn) => roundRobinEvents.filter((event) => event.source.line === line && event.frame.scheduler.turn === turn);
const rrState = (event) => event.frame.scheduler;
const rrLocation = (state, id) => [state.readyQueue.includes(id) ? 'READY' : null, state.cpu === id ? 'CPU' : null, state.moving?.id === id ? 'MOVING' : null, state.completedProcesses.includes(id) ? 'COMPLETED' : null].filter(Boolean);
ok('round robin preserves curriculum identity and expands both turns into immutable phases', roundRobinActivity.id === 'queue-round-robin' && roundRobinActivity.workspaceComposition === 'round-robin-execution' && roundRobinEvents.length === 47 && roundRobinEvents.every((event) => Object.isFrozen(event.frame.scheduler) && Object.isFrozen(event.frame.scheduler.readyQueue) && Object.isFrozen(event.frame.execution)));
ok('initial ready queue and configured quantum preserve FIFO arrival order', rrState(roundRobinEvents[0]).readyQueue.join() === 'P1,P2' && rrState(roundRobinEvents[0]).remainingByProcess.P1 === 5 && rrState(roundRobinEvents[0]).remainingByProcess.P2 === 2 && rrState(roundRobinEvents[0]).quantum === 2 && rrState(roundRobinEvents[0]).turn === 0);
ok('round robin executes all nine source lines with turn-specific operation phases', [1,2,3,4,5,6,7,8,9].every((line) => roundRobinEvents.some((event) => event.source.line === line)) && rrPhases(1,0).length === 4 && rrPhases(2,0).length === 1 && [3,4,5,6].every((line) => rrPhases(line,1).length === 4 && rrPhases(line,2).length === 4) && rrPhases(7,1).length === 4 && rrPhases(7,2).length === 0 && rrPhases(8,1).length === 1 && rrPhases(8,2).length === 1 && rrPhases(9,2).length === 4);
ok('each process has exactly one committed scheduler location in every phase', roundRobinEvents.every((event) => ['P1','P2'].every((id) => rrLocation(rrState(event), id).length === 1)));
const p1Dispatch = rrPhases(3,1);
ok('P1 dispatch stages Ready FRONT to CPU without consuming time', p1Dispatch[0].frame.scheduler.readyQueue.join() === 'P1,P2' && rrState(p1Dispatch[1]).moving.id === 'P1' && rrState(p1Dispatch[1]).readyQueue.join() === 'P2' && rrState(p1Dispatch[2]).cpu === 'P1' && rrState(p1Dispatch[2]).transition.kind === 'DISPATCH' && p1Dispatch.every((event) => rrState(event).remainingByProcess.P1 === 5) && rrState(p1Dispatch[3]).runtime.remaining === 5);
const p1Run = rrPhases(4,1), p1Update = rrPhases(5,1);
ok('line 4 computes a 2 ms P1 slice and previews 3 ms without committing remaining', p1Run[2].frame.scheduler.sliceDuration === 2 && p1Run[2].frame.scheduler.sliceResult === 3 && p1Run.every((event) => rrState(event).remainingByProcess.P1 === 5 && rrState(event).runtime.remaining === 5));
ok('line 5 commits P1 remaining only in its assignment phase', p1Update.slice(0,3).every((event) => rrState(event).remainingByProcess.P1 === 5 && rrState(event).runtime.remaining === 5) && rrState(p1Update[3]).remainingByProcess.P1 === 3 && rrState(p1Update[3]).runtime.remaining === 3);
const p1Check = rrPhases(6,1), p1Enqueue = rrPhases(7,1);
ok('P1 TRUE branch is visible before re-enqueue movement', p1Check[0].frame.scheduler.branchResult === null && p1Check[1].frame.scheduler.branchResult === true && p1Check.every((event) => rrState(event).cpu === 'P1' && rrState(event).readyQueue.join() === 'P2'));
ok('P1 moves behind P2 and next FRONT remains P2', rrState(p1Enqueue[1]).moving.id === 'P1' && rrState(p1Enqueue[1]).readyQueue.join() === 'P2' && rrState(p1Enqueue[2]).readyQueue.join() === 'P2,P1' && rrState(p1Enqueue[2]).remainingByProcess.P1 === 3 && rrState(p1Enqueue[3]).readyQueue[0] === 'P2');
const p2Dispatch = rrPhases(3,2), p2Run = rrPhases(4,2), p2Update = rrPhases(5,2), p2Check = rrPhases(6,2);
ok('second turn independently reuses line 3 and dispatches P2 while P1 stays ready', p2Dispatch[0].source.line === p1Dispatch[0].source.line && rrState(p2Dispatch[0]).turn === 2 && rrState(p2Dispatch[0]).readyQueue.join() === 'P2,P1' && rrState(p2Dispatch[2]).cpu === 'P2' && rrState(p2Dispatch[2]).readyQueue.join() === 'P1' && p2Dispatch.every((event) => rrState(event).remainingByProcess.P2 === 2 && rrState(event).remainingByProcess.P1 === 3));
ok('P2 RUN previews 2 to 0 while line 5 owns the committed remaining update', p2Run[2].frame.scheduler.sliceDuration === 2 && p2Run[2].frame.scheduler.sliceResult === 0 && p2Run.every((event) => rrState(event).runtime.remaining === 2 && rrState(event).remainingByProcess.P2 === 2) && p2Update.slice(0,3).every((event) => rrState(event).remainingByProcess.P2 === 2) && rrState(p2Update[3]).remainingByProcess.P2 === 0 && rrState(p2Update[3]).runtime.remaining === 0);
ok('P2 FALSE decision precedes CPU to Completed transfer and never enqueues P2', p2Check[1].frame.scheduler.branchResult === false && p2Check[2].frame.scheduler.cpu === 'P2' && rrState(p2Check[2]).completedProcesses.length === 0 && rrState(p2Check[3]).cpu === null && rrState(p2Check[3]).completedProcesses.join() === 'P2' && rrState(p2Check[3]).transition.kind === 'COMPLETE' && p2Check.every((event) => rrState(event).readyQueue.join() === 'P1'));
const rrReturn = rrPhases(9,2);
ok('RETURN separates Completed P2 from ready-queue program output', rrReturn.every((event) => rrState(event).readyQueue.join() === 'P1' && rrState(event).completedProcesses.join() === 'P2' && rrState(event).cpu === null) && rrReturn[1].frame.output.length === 0 && rrState(rrReturn[1]).returnValue.join() === 'P1 · 3 ms' && rrReturn[2].frame.output.join() === 'P1 · 3 ms' && rrState(rrReturn[3]).complete && rrState(rrReturn[3]).runtime.process === null && rrReturn[3].terminal);
const rrController = Playback.createController();
rrController.load(roundRobinEvents, roundRobinEvents.indexOf(p1Enqueue[2]));
rrController.step(1);
rrController.step(-1);
ok('round robin Previous restores queue CPU remaining turn phase branch output and transition', rrController.getState().currentEvent === p1Enqueue[2] && rrState(rrController.getState().currentEvent).readyQueue.join() === 'P2,P1' && rrState(rrController.getState().currentEvent).cpu === null && rrState(rrController.getState().currentEvent).remainingByProcess.P1 === 3 && rrState(rrController.getState().currentEvent).turn === 1 && rrController.getState().currentEvent.source.line === 7 && rrController.getState().currentEvent.frame.execution.phaseIndex === 2 && rrState(rrController.getState().currentEvent).branchResult === true && rrState(rrController.getState().currentEvent).transition.kind === 'REENQUEUE' && rrController.getState().currentEvent.frame.output.length === 0);
rrController.dispose();
const printerActivity = Activities.get('queue-printer-jobs');
const printerEvents = printerActivity.run().events;
const printerJobs = (event) => event.frame.lanes[0].items;
const printerHeld = (event, label) => event.frame.held.find((item) => item.label === label) || null;
ok('printer queue keeps its route identity and uses a printer-specific workspace', printerActivity.id === 'queue-printer-jobs' && printerActivity.workspaceComposition === 'printer-queue-execution' && printerActivity.title === 'Office printer queue' && printerActivity.breadcrumbLabel === 'Office printer queue');
ok('printer queue preserves seven source-ordered playback states', printerEvents.length === 7 && printerEvents.map((event) => event.source.line).join() === '1,2,3,4,5,6,7' && printerEvents.every((event) => Object.isFrozen(event.frame.lanes) && Object.isFrozen(event.frame.output)));
ok('printer queue starts empty with a capacity of three and no jobs or output', printerJobs(printerEvents[0]).length === 0 && printerEvents[0].frame.markers.variables.size === 0 && printerEvents[0].frame.output.length === 0);
ok('Report is staged as one incoming eight-page job before queue insertion', printerJobs(printerEvents[1]).length === 0 && printerHeld(printerEvents[1], 'incoming job')?.value === 'Report · 8 pages' && printerEvents[1].frame.markers.variables.size === 0);
ok('Form is staged behind the committed Report without changing FIFO order', printerJobs(printerEvents[2]).map((item) => item.value).join('|') === 'Report · 8 pages' && printerHeld(printerEvents[2], 'incoming job')?.value === 'Form · 1 page' && printerEvents[2].frame.markers.variables.front === 'Report' && printerEvents[2].frame.markers.variables.back === 'Report' && printerEvents[2].frame.markers.variables.size === 1);
ok('full printer queue remains Report, Form, Slides with semantic front and back', printerJobs(printerEvents[3]).map((item) => item.value).join('|') === 'Report · 8 pages|Form · 1 page|Slides · 4 pages' && printerEvents[3].frame.markers.variables.front === 'Report' && printerEvents[3].frame.markers.variables.back === 'Slides' && printerEvents[3].frame.markers.variables.size === 3 && /shorter Form does not jump/.test(printerEvents[3].message));
ok('DEQUEUE removes Report from the queue but keeps it as current', printerJobs(printerEvents[4]).map((item) => item.value).join('|') === 'Form · 1 page|Slides · 4 pages' && printerHeld(printerEvents[4], 'current job')?.value === 'Report · 8 pages' && printerEvents[4].frame.markers.variables.current === 'Report' && printerEvents[4].frame.output.length === 0);
ok('PRINT records Report exactly once as completed and exposes printed output', printerJobs(printerEvents[5]).map((item) => item.value).join('|') === 'Form · 1 page|Slides · 4 pages' && printerHeld(printerEvents[5], 'completed')?.value === 'Report · 8 pages' && printerEvents[5].frame.output.join() === 'Printed Report');
ok('RETURN clears current and returns the remaining jobs in FIFO order', printerJobs(printerEvents[6]).map((item) => item.value).join('|') === 'Form · 1 page|Slides · 4 pages' && !printerHeld(printerEvents[6], 'current job') && printerEvents[6].frame.output.join() === 'Printed Report' && printerActivity.run().result.remaining.join() === 'Form,Slides');
const printerController = Playback.createController();
printerController.load(printerEvents, 4);
printerController.step(1);
printerController.step(-1);
