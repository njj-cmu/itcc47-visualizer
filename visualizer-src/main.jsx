...
import { StackExecutionWorkspace } from './stack-execution.jsx';
import { PostfixExecutionWorkspace } from './postfix-execution.jsx';
import { DelimiterExecutionWorkspace } from './delimiter-execution.jsx';
import { UndoRedoExecutionWorkspace } from './undo-redo-execution.jsx';
import { QueueExecutionWorkspace } from './queue-execution.jsx';
...
function ContextualScenarioControls({ activity, inputs, setInputs, controller }) {
  ...
  return <div className="contextual-scenario-controls" aria-label="Scenario controls">
    <label><span>{activity.input.presetLabel || 'Case preset'}</span><select aria-label={activity.input.presetLabel || 'Case preset'} value={presetValue} onChange={(event) => changeInput('preset', event.target.value)}>{activity.input.presets.map((preset) => <option value={preset.id} key={preset.id}>{preset.label}</option>)}</select></label>
    <label><span>{activity.input.representationLabel || 'Representation'}</span><select aria-label={activity.input.representationLabel || 'Representation'} value={representationValue} onChange={(event) => changeInput('representation', event.target.value)}>{activity.input.representations.map((representation) => <option value={representation.id} key={representation.id}>{representation.label}</option>)}</select></label>
  </div>;
}
...
function VisualizerWorkspace({ params, courseId, requestedId }) {
  ...
  const isStackExecution = workspaceComposition === 'stack-execution';
  const isPostfixExecution = workspaceComposition === 'postfix-execution';
  const isDelimiterExecution = workspaceComposition === 'delimiter-execution';
  const isUndoRedoExecution = workspaceComposition === 'undo-redo-execution';
  const isQueueExecution = workspaceComposition === 'queue-execution';
  const isPhaseExecution = isStackExecution || isPostfixExecution || isDelimiterExecution || isUndoRedoExecution || isQueueExecution;
  ...
  function changeNetworkGranularity(next) {
    ...
    controller.pause();
    const operationId = event?.frame?.operation?.id || (isNetworkFoundations ? 'network-purpose' : 'evaluate-subnet');
  ...
  return <LazyMotion features={domMax} strict><MotionConfig reducedMotion={motionPreference.mode === 'on' ? 'never' : 'always'} transition={{ duration }}><div className={`visualizer-workspace course-${courseId} evidence-${workspaceLayout.evidence} motion-${motionPreference.mode} navigation-${playback.navigationSource} composition-${workspaceComposition}`} data-motion-duration={isComputerNetworking ? networkDuration : duration} data-workspace-composition={workspaceComposition}>
    <main className="workspace-main">
      ...
      {!isITCC45 && !isComputerArchitecture && !isComputerNetworking && !usesContextualControls && !isPostfixExecution && !isDelimiterExecution && !isUndoRedoExecution && !isQueueExecution ? <DataControls activity={activity} inputs={inputs} setInputs={setInputs} onShuffle={shuffle} controller={controller} viewOptions={viewOptions} setViewOptions={setViewOptions}/> : null}
      ...
      {isPhaseExecution ? <details className="stack-evidence-details"><summary>Learning Evidence · trace, variables, and operation counts</summary><EvidenceDrawer tab={evidenceTab} setTab={setEvidenceTab} activity={activity} result={result} event={event} index={playback.index} controller={controller} inputs={inputs} viewOptions={viewOptions} setViewOptions={setViewOptions}/></details> : null}
    ...
    {!usesCompactWorkspace && !isComputerNetworking && !isPhaseExecution ? <div className="desktop-evidence"><CollapsibleEvidencePanel contentId={isITCC45 ? 'itcc45-learning-evidence' : isComputerArchitecture ? 'computer-architecture-learning-evidence' : 'itcc47-learning-evidence'} expanded={workspaceLayout.evidence === 'expanded'} onExpandedChange={(expanded) => updateWorkspaceLayout({ evidence: expanded ? 'expanded' : 'collapsed' })} showCurrentLabel={isITCC45 || isComputerArchitecture} tab={evidenceTab} setTab={setEvidenceTab} activity={activity} result={result} event={event} index={playback.index} controller={controller} inputs={inputs} viewOptions={viewOptions} setViewOptions={setViewOptions}/></div> : null}
